...
async def _extract_override_from_reason(
    ...
    def _strip_tag(m: re.Match) -> str:
        ...
        return (
            ""
            if head in {"warn", "ban", "mute", "kick", "tban", "tmute"}
            else m.group(0)
        )
...
async def blocklist_action(client, message, blocklist_word):
    ...
    if not message.from_user:
        # Can't apply admin actions to sender chats (channels); just delete if
        # enabled.
    ...
    override_mode, override_time, cleaned_reason = await _extract_override_from_reason(
        reason
    )
