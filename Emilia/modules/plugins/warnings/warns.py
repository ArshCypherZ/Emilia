...
async def warns(client, message):
    ...
    is_admin = await isUserAdmin(message, pm_mode=True, silent=True)

    if is_admin and len(message.text.split()) >= 2:
        user_info = await get_user_id(message)
    else:
        if len(message.text.split()) >= 2 and not is_admin:
            return await message.reply("Only admins can check another user's warnings.")
