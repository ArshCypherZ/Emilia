...
from Emilia.modules.plugins.greetings.captcha import button_captcha, text_captcha
from Emilia.modules.plugins.greetings.group_onboarding import send_group_onboarding
...
from Emilia.utils.auth import is_dev, is_owner
    ...
    try:
        ...
        if await GetCleanWelcome(chat_id):
            ...
            if CleanWelcomeMessageID is not None:
                try:
                    ...
                    LOGGER.error(
                        f"[Welcome Debug] Failed to delete old welcome message: {e}"
                    )

        NewUserJson = (
            message.new_chat_member.user
        ...
        if user_id == BOT_ID:
            if getattr(client, "is_clone", False):
                await client.send_message(
                    chat_id=chat_id,
                    text="Welcome! You've just added a clone of the incredible @Elf_Robot. Thanks for choosing us! 🙌",
                )
                return
            await send_group_onboarding(client, chat_id)
            await client.send_message(
                chat_id=LOG_CHANNEL,
                text=(
                ...
                if not (await isReCaptcha(chat_id)):
                    ...
                    if not (
                        ...
                        try:
                            ...
                            LOGGER.error(
                                f"[Welcome Debug] Failed to restrict user: {e}"
                            )
                else:
                    if not await isUserAdmin(
                        ...
                        try:
                            ...
                            LOGGER.error(
                                f"[Welcome Debug] Failed to restrict user (recaptcha): {e}"
                            )
            else:
        ...
        if await isWelcome(chat_id):
            ...
            try:
                ...
                LOGGER.error(
                    f"[Welcome Debug] Failed to send default welcome message: {e}"
                )
    except Exception as e:
