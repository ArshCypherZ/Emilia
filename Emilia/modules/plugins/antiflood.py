...
# Caches
_flood_status_cache = SimpleCache(default_ttl=60, namespace="antiflood_status")
# Antiflood Settings Cache: L1 (Memory) -> L2 (Redis) -> L3 (Mongo)
_settings_cache = SimpleCache(default_ttl=300, namespace="antiflood_settings")
...
async def _is_approved_cached(chat_id: int, user_id: int) -> bool:
    ...
    if val is not None:
        return val
    is_approved = (
        await db["approve_d"].find_one({"user_id": user_id, "chat_id": chat_id})
        is not None
    )
...
async def setfloodtimer(client, message):
    ...
    await DB.update_one(
        ...
        {
            "$set": {
                "timed_limit": int(count),
                "timed_duration": int(duration),
                "timed_status": "on",
            }
        },
        upsert=True,
...
async def handle_message(client, message):
    ...
    if await _is_approved_cached(chat_id, user_id):
        if last_user_id and int(last_user_id) != user_id:
            ...
            # Clear Regular User's ZSET to be lenient (reset their strict
            # consecutive count).
    ...
    if timed_on:
        ...
        # Consecutive count
        # By clearing the ZSET on user switch, ZCARD is effectively consecutive
        # count
            ...
            if action == "ban":
                ...
                if action == "tban":
                    await client.ban_chat_member(
                        chat_id, user_id, until_date=time_value
                    )
                    await client.send_message(
                        chat_id,
                        f"{firstname} has been temporarily banned ({time_str}) for spamming.",
                    )
                else:
                    until_ts = (
                        int(time_value.timestamp())
                        if hasattr(time_value, "timestamp")
                        else int(time_value)
                    )
                    if not await restrict_member_no_reactions(
                        chat_id, user_id, until_date=until_ts
                    ):
                        await client.restrict_chat_member(
                            chat_id,
                            user_id,
                    ...
                    await client.send_message(
                        chat_id,
                        f"{firstname} has been temporarily muted ({time_str}) for spamming.",
                    )
