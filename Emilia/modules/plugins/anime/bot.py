...
from Emilia.utils.helper import AUTH_USERS, check_user, control_user
...
async def en_dis__able_cmd(client: Client, message: Message, mdata: dict):
    ...
    if (
        user in DEV_USERS
        or (await client.get_chat_member(gid, user)).status
        ...
        if set(cmd[1].split()).issubset(CMD):
            ...
            if find_gc is None:
                ...
                await DC.update_one(
                    {"_id": gid}, {"$setOnInsert": {"cmd_list": cmd[1]}}, upsert=True
                )
...
async def connect_(client: Client, message: Message, mdata: dict):
    ...
    if gid == id_:
        ...
        if data[0] == "aniconnect":
            ...
            # Idempotent connect
            await CC.update_one(
                {"_id": str(channel)}, {"$setOnInsert": {"usr": id_}}, upsert=True
            )
...
async def aexec(code, client, message):
    # DEV_USERS-gated by filters.user(DEV_USERS) on the callers above — do not
    # loosen.
    exec(
...
async def terminal(client: Client, message: Message, mdata: dict):
    # DEV_USERS-gated by filters.user(DEV_USERS) above — do not loosen.
    ...
    if "\n" in teks:
        ...
        for x in code:
            ...
            try:
                ...
                LOGGER.error(err)
                await message.reply_text(
                    f"<b>Error:</b>\n<pre>{html.escape(str(err))[:4000]}</pre>",
        ...
        try:
            ...
            if stderr:
                ...
                await message.reply_text(
                    f"<b>Error:</b>\n<pre>{html.escape(''.join(errors))[:4000]}</pre>",
                    parse_mode=enums.ParseMode.HTML,
                )
                return
        except Exception:
    ...
    if output:
        if len(output) > 4096:
            ...
            return
        await message.reply_text(
            f"<b>Output:</b>\n<pre>{html.escape(str(output))[:4000]}</pre>",
            parse_mode=enums.ParseMode.HTML,
        )
    else:
