...
async def anime_cmd(client: Client, message: Message, mdata: dict):
    ...
    gid = mdata["chat"]["id"]
    try:
        await client.send_chat_action(gid, ChatAction.TYPING)
    except Exception:
        pass
    try:
...
async def settings_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        status = (await client.get_chat_member(cid, user)).status
...
async def nsfw_toggle_btn(client: Client, cq: CallbackQuery):
    ...
    if query[1] == "sfw":
        if await SFW_GRPS.find_one({"id": int(query[2])}):
            ...
            await SFW_GRPS.update_one(
                {"id": int(query[2])},
                {"$setOnInsert": {"id": int(query[2])}},
                upsert=True,
            )
            sfw = "NSFW: Not Allowed"
    if query[1] == "sp":
    ...
    if kek == "char":
        ...
        if btndata["lastPage"] != 1:
            ...
            if page == "1":
                ...
                button.append([InlineKeyboardButton(text="《", callback_data=pre)])
            else:
                button.append(
                    [
                        InlineKeyboardButton(text="《", callback_data=pre),
...
async def featured_in_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if total > 15:
        button.append(
            [
                InlineKeyboardButton(
                    text="》", callback_data=f"lsca_{idm}_1_{qry}_{pg}_{auth}_{usr}"
                )
            ]
        )
    if req is not None:
    ...
    if re.match(r"^malhd$", qry):
        if data:
            ...
            await collection.update_one(
                {"_id": gid}, {"$setOnInsert": {"_id": gid}}, upsert=True
            )
...
async def auto_unpin(client: Client, cq: CallbackQuery):
    ...
    for i in TIMES.keys():
        count = count + 1
        if i == "New Feed":
            row.append(InlineKeyboardButton(text=i, callback_data=f"unpin_0_mal_{gid}"))
        else:
            if i == "OFF":
                row.append(
                    InlineKeyboardButton(text=i, callback_data=f"unpin_None_mal_{gid}")
                )
            else:
                row.append(
                    InlineKeyboardButton(
...
async def change_ui_btn(client: Client, cq: CallbackQuery):
    ...
    if qry in ["Caps", "UPPER"]:
        if await GUI.find_one({"_id": gid}):
            ...
            await GUI.update_one(
                {"_id": gid}, {"$setOnInsert": {"bl": "➤", "cs": qry}}, upsert=True
            )
    elif qry != "call":
        bullet = qry
        if qry == "None":
        ...
        if await GUI.find_one({"_id": gid}):
            ...
            await GUI.update_one(
                {"_id": gid},
                {"$setOnInsert": {"bl": bullet, "cs": "UPPER"}},
                upsert=True,
            )
    bl = "➤"
    cs = "UPPER"
