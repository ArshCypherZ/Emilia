...
async def increase_points(user_id, chat_id, points):
    # Use targeted filter and upsert to avoid read-before-write; ensured index
    # on (chat_id, user_id)
    flt = {"user_id": int(user_id), "chat_id": int(chat_id)}
    return await users_collection.update_one(
        flt, {"$inc": {"points": points}}, upsert=True
    )
...
async def upvote(_, message):
    ...
    if is_karma:
        ...
        fuser = await users_collection.find_one(
            {"user_id": int(user_id), "chat_id": int(chat_id)}, {"points": 1}
        )
