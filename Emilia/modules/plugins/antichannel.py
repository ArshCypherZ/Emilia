...
async def antichannelmode_on(chat_id: int):
    return await antichanneldb.update_one(
...
@Client.on_message(
    custom_filter.command(commands=["antichannel", "antichannelmode"]) & filters.group
)
@log_to_channel
@anonadmin_checker
    ...
    if message.sender_chat and message.sender_chat.type == enums.ChatType.CHANNEL:
        ...
        try:
            ...
            if linked_channel:
                ...
                if fwd_chat_ and fwd_chat_.id == linked_channel:
                    return
