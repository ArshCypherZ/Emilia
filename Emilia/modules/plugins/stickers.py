...
def get_emoji_regex():
    # Kurigram dropped its bundled `pyrogram.emoji` constants module upstream
    # (commit 707de859, "Remove emoji module"); source the character set from
    # the `emoji` PyPI package's data table instead.
    e_list = [c.encode("unicode-escape").decode("ASCII") for c in emoji.EMOJI_DATA]
...
def resize_image_sync(filename: str) -> str:
    ...
    if (im.width, im.height) != (512, 512):
        if im.width == im.height:
            ...
            sizenew = (512, int(im.height * 512 / im.width))
        else:
            sizenew = (int(im.width * 512 / im.height), 512)
        im = im.resize(sizenew, Image.Resampling.LANCZOS)
