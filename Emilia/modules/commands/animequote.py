...
async def quotes(client, message):
    try:
        anim: str = message.text.split(None, 1)[1]
...
async def animequotes(client, message):
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("Change 🔁", callback_data="changek_quote")]]
    )
    await client.send_photo(
        message.chat.id, random.choice(QUOTES_IMG), reply_markup=keyboard
    )


@callbackquery(pattern=r"changek_.*")
@callbackquery(pattern=r"quotek_.*")
async def changek_quote(client, query):
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("Change 🔁", callback_data="quotek_change")]]
    )
    # editing media requires edit_message_media in pyrogram
    await query.edit_message_media(
        InputMediaPhoto(random.choice(QUOTES_IMG)), reply_markup=keyboard
    )
