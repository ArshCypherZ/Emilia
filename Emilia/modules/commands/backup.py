...
from Emilia import EVENT_LOGS as CHANNEL_ID
from Emilia import LOGGER, db, pgram
...
async def _send_with_retry(path: str, caption: str | None = None, max_retries: int = 5):
    # pyrogram manages upload chunk size internally
    for attempt in range(max_retries):
        try:
            ...
            LOGGER.error(f"FloodWait during sending {path}: {e.value}s")
            await asyncio.sleep(e.value + 2)
        except asyncio.TimeoutError as e:
            LOGGER.error(
                f"Timeout sending {path} (attempt {attempt+1}/{max_retries}): {e}"
            )
...
# Single approach: Python zip implementation (no external `zip`, no splitting)
