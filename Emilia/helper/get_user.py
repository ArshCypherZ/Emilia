...
from Emilia.helper.forward_origin import fwd_user as _fwd_user
...
async def get_user_id(message):
    ...
    fwd_user = _fwd_user(message)

    if message.reply_to_message and not fwd_user:
        ...
        # Fallback to legacy split check if no entities found but arguments
        # present
        if len(text.split()) >= 2:
            args = text.split()[1]
            # If just an ID or username string without entity (rare but
            # possible)
    ...
    elif not (message.reply_to_message or fwd_user):
